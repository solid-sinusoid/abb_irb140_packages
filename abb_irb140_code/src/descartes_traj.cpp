...
int main(int argc, char** argv)
{
  ...
  // "manipulator"
  const std::string group_name = "my_abb_groups";
...
std::vector<descartes_core::TrajectoryPtPtr> makePath()
{
  ...
  const static double o_rad = 0.05;
  const static double O_rad = 0.1;
  const static double h = 0.3;
  const static double m = 0.2;
  double degree = 3.14;
  double x = 0;
  double y = 0;
  ...
  for (double i = 0; i < degree; i=i+0.1) {
    Eigen::Isometry3d pose = Eigen::Isometry3d::Identity();
    x = O_rad*(m+1)*cos(m*i)+h*cos((m+1)*i);
    y = O_rad*(m+1)*sin(m*i)+h*sin((m+1)*i);

    pose.translation() = Eigen::Vector3d(x,y,0);
